...
class MopacTS(QMReaction, Mopac):
    ...
    def generateTransitionState(self):
        ...
        with open(inputFilePath, 'w') as mopacFile:
            mopacFile.write(self.family.name)
            mopacFile.write('degeneracy = ')
            mopacFile.write(str(self.reaction.degeneracy))
            mopacFile.write('\n')
            for action in self.family.forwardRecipe.actions:
                mopacFile.write(str(action))
                mopacFile.write('\n')
